...
def benchmark_raycast_engine(
    ...
    metrics = {
        ...
        "speedup_factor": (100 / single_ray_time) / (num_rays / batch_ray_time) 
                          if batch_ray_time > 0 and single_ray_time > 0 else 0
    }
            ...
            if not isinstance(embree_engine, type(standard_engine)):  # Different engine types
                ...
                results["comparison"] = {
                    "embree_vs_standard_speedup": (
                        embree_metrics["rays_per_second_batch"] / 
                        standard_metrics["rays_per_second_batch"]
                    ) if standard_metrics["rays_per_second_batch"] > 0 else 0
                }
        except ImportError:
