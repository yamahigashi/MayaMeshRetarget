...
from .benchmark import (
    benchmark_raycast_engines_comparison,
...
def run_registration_comparison(
    ...
    print(f"BVH + Multithreading: {results['optimal']['avg_time']:.2f} seconds (Speedup: {results['optimal'].get('speedup', 0):.2f}x)")
...
if __name__ == "__main__":
    ...
    if len(selection) < 1:
        cmds.error("Please select at least a source mesh for engine comparison. For full registration comparison, select both source and target meshes.")
