...
def test_registration_options():
    ...
    assert options.align_spaces is True
    assert options.alignment_method == "umeyama"
    assert options.use_partial_matching is True
    assert options.use_hierarchical_matching is True
    assert options.match_confidence_threshold == 0.5
    assert options.max_points_per_target == 1
    assert options.use_scoring_components is True
    ...
    assert custom_options.use_partial_matching is False
